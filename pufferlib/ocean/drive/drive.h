...
void compute_observations(Drive *env) {
    ...
    int max_obs =
        ego_dim + PARTNER_FEATURES * (MAX_AGENTS - 1) + ROAD_FEATURES * MAX_ROAD_SEGMENT_OBSERVATIONS + target_dim;
    ...
    for (int i = 0; i < env->active_agent_count; i++) {
        ...
        if (target_dim > 0) {
            ...
            if (env->active_agent_indices[i] != env->sdc_track_index) {
                ...
                float rel_heading_x =
                    target_entity->heading_x * ego_entity->heading_x + target_entity->heading_y * ego_entity->heading_y;
                float rel_heading_y =
                    target_entity->heading_y * ego_entity->heading_x - target_entity->heading_x * ego_entity->heading_y;
                ...
                // relative speed
                float target_speed =
                    sqrtf(target_entity->vx * target_entity->vx + target_entity->vy * target_entity->vy);
        ...
        if (sdc_active_index != -1) {
            ...
            for (int i = 0; i < env->active_agent_count; i++) {
                if (i != sdc_active_index) {
                    ...
                    // env->rewards[i] = -( dx * dx + dy * dy) / 1000000.0;
